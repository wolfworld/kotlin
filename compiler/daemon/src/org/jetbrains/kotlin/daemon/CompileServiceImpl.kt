...
class CompileServiceImpl(
    ...
    private inline fun ifAliveUnit(minAliveness: Aliveness = Aliveness.LastSession, body: () -> Unit): Unit = rwlock.read {
        ifAliveChecksImplUnit(minAliveness, body)
    }
    ...
    private inline fun ifAliveChecksImplUnit(minAliveness: Aliveness = Aliveness.LastSession, body: () -> Unit): Unit {
        val curState = state.alive.get()
        if (curState < minAliveness.ordinal) log.info("Cannot perform operation, requested state: ${minAliveness.name} > actual: ${curState.toAlivenessName()}")
        else {
            try {
                body()
            }
